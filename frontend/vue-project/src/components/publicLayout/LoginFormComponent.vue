...
const enviarDatos = () => {
  console.log('enviarDatos ejecutado', form.value)
  emit('submit-login', { ...form.value })
}
</script>

<template>
  <b-card header="Acceso al Sistema" header-bg-variant="primary" header-text-variant="white" class="shadow-sm">
    <b-form @submit.prevent="enviarDatos">
      <b-form-group label="Correo Electrónico:" label-for="input-email" class="mb-3">
        <b-form-input id="input-email" v-model="form.email" type="email" placeholder="ejemplo@correo.com"
      ...
      <b-form-group label="Contraseña:" label-for="input-password" class="mb-3">
        <b-input-group>
          <b-form-input id="input-password" v-model="form.password" :type="showPassword ? 'text' : 'password'"
            placeholder="Ingresa tu contraseña" required></b-form-input>
          <b-input-group-text>
